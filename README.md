...
- SmartArt 内容过多时自动排成同页多行，避免过度压缩和遮挡
...
当单页 SmartArt 节点过多时，程序会自动在同一页中拆成多行上下排列，避免节点过窄、文字重叠或时间线过密。

当前默认换行阈值：

| SmartArt 类型 | 同页换行阈值 |
| --- | ---: |
| `process` / `chevron` | 超过 5 个顶层节点时换行 |
| `process` / `numbered` | 超过 5 个顶层节点时换行 |
| `timeline` | 超过 6 个时间节点时换行 |
| `cycle` | 超过 6 个节点时换行 |
| `pyramid` | 超过 5 层时换行 |
| `matrix` | 不自动换行 |
| `hierarchy` | 不自动换行 |
...
补充规则：如果仅比阈值多 1 个节点，程序不会把最后 1 个节点单独拆成一页，而会从前一页借出 1 个节点。例如：

```text
6 个 chevron 节点：4 + 2
11 个 chevron 节点：5 + 4 + 2
16 个 chevron 节点：5 + 5 + 4 + 2
```

同页多行排列不会修改幻灯片标题；例如：

```text
6 个 chevron：4 + 2，上下两行
11 个 chevron：5 + 4 + 2，上中下三行
```

对于 `hierarchy`，程序默认不自动换行，因为树形结构需要保持父子关系。  
如果组织架构过大，建议人工拆分为多个 `hierarchy` 图，或改为普通列表/表格说明。
